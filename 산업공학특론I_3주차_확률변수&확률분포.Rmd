...
### 1. 데이터 탐색 (EDA) 및 전처리
```{r data_eda}
getwd() #작업공간 확인하는 것 일치하지 않으면 setwd(") 사용
list.files(pattern = "csv") #csv가 붙은 파일 명 찾는 법
dat <- read.csv("산업공학특론I_3주차_실습 데이터.csv")
head(dat) #UDI나 Product.ID는 관련 없으므로 삭제

#불필요한 변수 삭제
dat <- dat[,-c(1:2)] #1열부터 2열까지 삭제해서 다시 저장
print(head(dat))

summary(dat)
dat$Target <- as.factor(dat$Target) #타겟을 변수형으로 변환해 #서머리 해보니 타겟이 0과 1이 몇개인지 나타남
...
```{r data_group}
dat_split <- split(dat$Tool.wear..min., dat$Target) #타겟에 따라서 데이터를 나눠보기
dat_split
summary(dat_split[[1]])
summary(dat_split[[2]]) #위랑 분포가 좀 차이남
```
...
print(fitdist(normal, 'exp'))
print(fitdist(fail,'exp'))
