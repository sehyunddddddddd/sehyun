...
lapply(dat_product, summary) #list안에 들어있는 모든 원소들에게 어떤 것을 수행해달라 , summary 라는 함수도 사용해달라
...
fit <- function(x){
  ...
  for (i in 1:length(x)){
    ...
    row.names(result) <- c('par1','par2','loglike') #여기를 loglk를 aic로 바꿔도 되고 아니면 $aic해도 나옴.
    ...
    best <- colnames(result)[which.max(result[3,])] #aic면 which.min으로 바꿔줘야함. 
